...
{
    public class CreateDelivererUnitTests
    {
        ...
        public CreateDelivererUnitTests()
        {
        ...
        [Fact]
        public async Task CreateDeliverer_Returns201_WhenValidData()
        {
            // Arrange
            ...
            // Act
            var result = await _controller.CreateDeliverer(delivererDto);

            // Assert
            var actionResult = Assert.IsType<StatusCodeResult>(result);
            Assert.Equal(201, actionResult.StatusCode);
            _delivererServiceMock.Verify(s => s.CreateDelivererAsync(delivererDto), Times.Once);
        }

        [Fact]
        public async Task CreateDeliverer_Returns400_WhenModelStateIsInvalid()
        {
            // Arrange
            _controller.ModelState.AddModelError("Identificador", "Required");
            ...
            _delivererServiceMock.Verify(s => s.CreateDelivererAsync(It.IsAny<DelivererDto>()), Times.Never);
        }

        [Fact]
        public async Task CreateDeliverer_Returns400_WhenServiceThrowsException()
        {
            // Arrange
            var delivererDto = new DelivererDto
            {
            ...
            _delivererServiceMock
                .Setup(s => s.CreateDelivererAsync(delivererDto))
